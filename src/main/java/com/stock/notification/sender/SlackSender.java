...
/** Slack通知发送器 */
@Component
public class SlackSender implements BaseNotificationSender {
    private static final Logger log = LoggerFactory.getLogger(SlackSender.class);
    private final AppConfig config;
    private final OkHttpClient httpClient;
    private final ObjectMapper objectMapper;

    public SlackSender(AppConfig config) {
        this.config = config;
    ...
    public boolean send(String title, String content) {
        ...
        } catch (Exception e) { log.error("Slack通知发送失败: {}", e.getMessage()); return false; }
    }
}
