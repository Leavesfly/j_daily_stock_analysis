...
public class CsvImportService {
     ...
     * 解析CSV文件，返回预览数据
     */
    public Map<String, Object> parseCsv(String broker, byte[] content, String filename) {
        ...
        try (Reader reader = new InputStreamReader(new ByteArrayInputStream(content), detectCharset(content))) {
            ...
            log.error("CSV解析失败: broker={}, file={}", broker, filename, e);
        }
     ...
     * 提交CSV导入，创建交易和现金流水记录
     */
    public Map<String, Object> commitCsv(Long accountId, String broker, byte[] content, String filename, boolean dryRun,
